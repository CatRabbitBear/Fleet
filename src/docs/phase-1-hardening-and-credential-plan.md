...
> Status update (April 6, 2026): the core Phase 1 framework has been implemented (request identity, policy gate, consent flow, session-token auth on privileged routes, and audit logging). The remaining close-out work is architecture extraction so filesystem-capable agent runtime logic can live outside Blazor in a sibling runtime project.

> Recommended next abstraction: create `src/Fleet.Runtime` (or `src/Fleet.Agents.Runtime`) and move agent orchestration/tool contracts there. Keep privileged filesystem/process execution adapters host-owned in Tray.
